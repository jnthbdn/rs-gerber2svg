...
use gerber_types::{Aperture, Command, CommentContent, CoordinateFormat, CoordinateNumber, Coordinates, GCode, InterpolationMode};
...
// Public re-export of svg crate to be used by dependencies
pub use svg;
...
impl Gerber2SVG {
    ...
    pub fn from_file(filename: &str) -> Result<Self, std::io::Error> {
        ...
        let gerber_doc: GerberDoc = parse(reader).map_err(|e| std::io::Error::new(
            ErrorKind::InvalidData,
            format!("Failed to parse Gerber file '{}': {}", filename, e.0),
        ))?;
    ...
    /// Get SVG as String
    /// * crop: `bool` trim unused space
    pub fn as_string(&mut self, crop: bool) -> String {
        self.set_bbox(crop);
        self.svg_document.to_string()
    }
    
    /// Get SVG Document
    pub fn document(&self) -> &svg::Document {
        &self.svg_document
    }
    ...
    pub fn build(mut self) -> Self {
        ...
        let commands = self.gerber_doc.commands().iter().map(|c| (*c).clone()).collect::<Vec<_>>();
        for c in commands {
            match c {
                    ...
                    match f {
                        FunctionCode::DCode(d) => match d {
                            gerber_types::DCode::Operation(o) => match o {
                                gerber_types::Operation::Interpolate(Some(coord), offset) => {
                        ...
                        FunctionCode::GCode(g) => match g {
                            ...
                            GCode::Comment(CommentContent::String(c)) => log::info!("[COMMENT] \"{}\"", c),
    ...
    fn place_aperture(&mut self, coord: &Coordinates) -> () {
            ...
            .expect("No aperture selected")
        {
            ...
            Aperture::Macro(_, m) => log::error!("Unsupported Other aperture:\r\n{:#?}", m),
        }
    ...
    fn coordinate_to_float(coord: &Coordinates) -> (Option<f32>, Option<f32>) {
        (
            Self::coordinate_number_to_float(&coord.x, &coord.format),
            Self::coordinate_number_to_float(&coord.y, &coord.format)
        )
    }
    ...
    fn coordinate_number_to_float(num: &Option<CoordinateNumber>, format: &CoordinateFormat) -> Option<f32> {
         num.map(|x| {
             f64::from(x) as f32
        })
    }
